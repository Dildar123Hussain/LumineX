...
export function Spinner({ size=20, color }) {
  return <div style={{ width:size,height:size,borderRadius:"50%",border:`2px solid ${color||C.accent}33`,borderTopColor:color||C.accent,animation:"spin .7s linear infinite",flexShrink:0 }}/>;
}
...
export function Input({ label, type="text", value, onChange, placeholder, icon, error, hint, autoFocus, onKeyDown, readOnly, right, onBlur }) {
  ...
  return (
    <div style={{ marginBottom:16 }}>
      {label&&<label style={{ display:"block",fontSize:11,fontWeight:700,color:C.muted,marginBottom:6,textTransform:"uppercase",letterSpacing:.5 }}>{label}</label>}
      <div style={{ position:"relative" }}>
        ...
        <input type={isPass&&!show?"password":"text"} value={value} onChange={onChange} placeholder={placeholder} autoFocus={autoFocus} onKeyDown={onKeyDown} readOnly={readOnly}
          onFocus={()=>setFocused(true)} onBlur={()=>{setFocused(false);onBlur&&onBlur();}}
          style={{ width:"100%",background:C.bg3,border:`1.5px solid ${displayErr?C.red:focused?C.accent:C.border}`,borderRadius:10,color:C.text,fontFamily:"inherit",fontSize:14,padding:`11px ${(isPass||right)?40:14}px 11px ${icon?40:14}px`,outline:"none",transition:"border-color .2s,box-shadow .2s",boxShadow:focused?`0 0 0 3px ${displayErr?C.red:C.accent}1a`:"none" }}/>
        {isPass&&<button type="button" onClick={()=>setShow(s=>!s)} style={{ position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:C.muted,fontSize:15,padding:4 }}>{show?"🙈":"👁"}</button>}
        {right&&!isPass&&<div style={{ position:"absolute",right:10,top:"50%",transform:"translateY(-50%)" }}>{right}</div>}
      </div>
      {displayErr&&<div style={{ fontSize:11,color:C.red,marginTop:4 }}>⚠ {displayErr}</div>}
...
export function Btn({ children, onClick, variant="primary", size="md", disabled, loading, fullWidth, style:s }) {
  ...
  return (
    ...
    >{loading?<Spinner size={14} color="white"/>:children}</button>
  );
}
